 ...
 *
 * Complements the humanity eval (warmth). This measures DISCIPLINE. Because we assign
 * the archetype, we know the ground truth:
 *   - SHOULD-unlock (engaged, correct_then_own, mixed_strand, shifting): unlocking is GOOD.
 *   - SHOULD-NOT (uncertain, terse, shallow_agreement, resistant, savouring): any unlock
 *     is a FALSE unlock (the user never owned a feeling).
 *   - NEUTRAL (not_quite): tracked, not scored.
 * Reports true/false-unlock rates, per-archetype unlock rate, the five-way outcome
 * taxonomy, and safety-pause count, per model.
 *
 * Prereqs: ai-proxy (:8787) + companion-server (:8788) running. Model set per request.
 * Run:  node eval/unlock-ab.mjs <label> --model=gpt-5.6-terra [--full] [--turns=8]
 *       --full = the full ~150-case matrix (mirrors matrix-sim); else a balanced 21-set.
 */
...
const TURNS = Number((process.argv.find((a) => a.startsWith('--turns=')) || '').split('=')[1]) || 8;
...
const ARCH = {
  ...
  mixed_strand: 'You have TWO real feelings at once (e.g. relief AND grief, pride AND loss, anger AND hurt). You refuse to collapse it to one: "its both", "at the same time", "part of me X and part of me Y". Hold both as you talk, and give real detail for each.',
  shifting: 'You START at the seed feeling but it is the SURFACE. As the companion stays with you, let the real feeling MOVE underneath to a different one (e.g. anger -> shame -> hurt), a layer at a time. Each layer is genuine and you own it in your own words.',
...
const SHOULD = new Set(['engaged_deepener', 'correct_then_own', 'mixed_strand', 'shifting']);
const SHOULD_NOT = new Set(['uncertain', 'terse', 'shallow_agreement', 'resistant', 'savouring']);

// Full matrix mix (mirrors eval/matrix-sim.mjs): ~15 archetypes per emotion.
function mixFor(emotion) {
  const rep = (k, n) => Array.from({ length: n }, () => k);
  if (emotion === 'joy' || emotion === 'calm')
    return [...rep('engaged_deepener', 3), ...rep('savouring', 3), ...rep('mixed_strand', 2), ...rep('uncertain', 1), ...rep('terse', 1), ...rep('shifting', 1), ...rep('not_quite', 1), ...rep('shallow_agreement', 1), ...rep('resistant', 1), ...rep('correct_then_own', 1)];
  if (emotion === 'mixed')
    return [...rep('mixed_strand', 4), ...rep('shifting', 3), ...rep('engaged_deepener', 3), ...rep('uncertain', 1), ...rep('not_quite', 1), ...rep('terse', 1), ...rep('shallow_agreement', 1), ...rep('correct_then_own', 1)];
  return [...rep('engaged_deepener', 3), ...rep('uncertain', 2), ...rep('terse', 2), ...rep('not_quite', 2), ...rep('shifting', 2), ...rep('mixed_strand', 1), ...rep('shallow_agreement', 1), ...rep('resistant', 1), ...rep('correct_then_own', 1)];
}
const BALANCED = [
  ...
  ['flat', 'terse'], ['anger', 'terse'], ['sadness', 'resistant'], ['fear', 'resistant'],
...
async function run([emotion, archetype], i) {
  const cid = `mx__${label}__${emotion}__${archetype}__${String(i).padStart(3, '0')}`;
  const transcript = [];
  let unlocked = false, unlockTurn = null, deepest = 'noticed', safety = false;
  const outcomes = [];
  for (let turn = 0; turn < TURNS; turn++) {
    let userMsg; try { userMsg = await persona(emotion, archetype, transcript); } catch { break; }
    if (!userMsg) break;
    let r; try { r = await say(cid, userMsg); } catch { break; }
    transcript.push({ role: 'user', content: userMsg });
    if (r.safety) { safety = true; transcript.push({ role: 'companion', content: '[safety pause]' }); break; }
  ...
  const rec = { cid, emotion, archetype, unlocked, unlockTurn, deepest, safety, finalOutcome: outcomes[outcomes.length - 1] ?? null };
...
async function pool(items, n, fn) {
  ...
  await Promise.all(Array.from({ length: Math.min(n, items.length) }, async () => { while (i < items.length) { const idx = i++; out[idx] = await fn(items[idx], idx); done++; if (done % 10 === 0) console.log(`  ...${done}/${items.length}`); } }));
  return out;
}

console.log(`Unlock ${FULL ? 'FULL matrix' : 'balanced'} "${label}": ${SPECS.length} cases × up to ${TURNS} turns · companion=${MODEL || 'server default'}\n`);
const recs = (await pool(SPECS, CONC, run)).filter(Boolean);

const should = recs.filter((r) => SHOULD.has(r.archetype));
const shouldNot = recs.filter((r) => SHOULD_NOT.has(r.archetype));
const byArch = {};
for (const r of recs) { const a = (byArch[r.archetype] ??= { n: 0, u: 0 }); a.n++; if (r.unlocked) a.u++; }
const outc = {};
for (const r of recs) outc[r.finalOutcome ?? 'null'] = (outc[r.finalOutcome ?? 'null'] || 0) + 1;
const summary = {
  model: MODEL || 'server default', cases: recs.length,
  true_unlock: `${should.filter((r) => r.unlocked).length}/${should.length}`,
  true_unlock_rate: should.length ? +(should.filter((r) => r.unlocked).length / should.length).toFixed(2) : null,
  false_unlock: `${shouldNot.filter((r) => r.unlocked).length}/${shouldNot.length}`,
  false_unlock_rate: shouldNot.length ? +(shouldNot.filter((r) => r.unlocked).length / shouldNot.length).toFixed(2) : null,
  overall_unlock_rate: +(recs.filter((r) => r.unlocked).length / recs.length).toFixed(2),
  safety_pauses: recs.filter((r) => r.safety).length,
  by_archetype: Object.fromEntries(Object.entries(byArch).map(([k, v]) => [k, `${v.u}/${v.n} (${(v.u / v.n).toFixed(2)})`])),
  final_outcomes: outc,
