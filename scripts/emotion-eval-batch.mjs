...
const BATCH = ['shame', 'flat'];
const COUNT = 24;
// ──────────────────────────────────────────────────────────────────────

const DESC = {
  joy: 'joy / excitement / delight / contentment / pride',
  calm: 'calm / peace / relief / feeling safe and settled',
  fear: 'fear / anxiety / worry / dread / nervousness',
  pressure: 'pressure / overwhelm / stress / being stretched too thin',
  anger: 'anger / frustration / irritation / resentment / protest',
  sadness: 'sadness / grief / loss / low mood / disappointment',
  hurt: 'hurt / feeling let down, rejected or betrayed by someone',
...
const MODES = [
  'engages openly with the companion', 'just wants to log the feeling and leave quickly',
  'opens with small talk or a greeting before the real feeling surfaces', 'mislabels the feeling at first (calls it something it is not)',
  'has two tangled feelings at once', 'is vague and resistant, keeps saying they do not know',
  'is a bit skeptical of the whole app', 'is quietly overwhelmed and hard to draw out',
];
const DOMAINS = [
...
function plan(i) {
  ...
  return { complexity, turns, style: STYLES[i % STYLES.length], mode: MODES[(i * 3 + 1) % MODES.length], domain: DOMAINS[(i * 7) % DOMAINS.length] };
}

function distressNote(emotion, i) {
  if (HEAVY.includes(emotion) && (i === 0 || i === 1)) {
